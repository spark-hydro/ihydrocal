...
def main():
    ...
    config_file = Path(
        r"C:\Users\spark\Documents\projects\watersheds\pecos\calibration_v03\config\setup_swatplus.yml"
    )
    ...
    base_channel_output = workspace_dir / "pecos_rw_swp_default" / "channel_sdmorph_day.txt"
    ...
    extract_swatplus_channel_output_long(
        ...
        cha_ids=[81],  # outlet only
        id_col=channel_cfg["id_col"],
        chunksize=200_000,
    )

    scenario_channel_output = workspace_dir / "pecos_rw_swp_prepared" / "channel_sdmorph_day.txt"
    ...
    fig, ax, fdc_df = plot_simulated_fdc_comparison(
        ...
        save_path=workspace_dir / "scenario_compare" / "outlet_base_vs_recall_fdc.png",
    )
