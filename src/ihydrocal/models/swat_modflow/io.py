...
class SWATModflowIO:
    ...
    def get_streamflow_pair(
        ...
        """Read and align simulated and observed streamflow.

        Parameters
        ----------
        obs_file : str or pathlib.Path
            Wide observed-streamflow CSV. The first column must contain dates.
        reach_id : int
            SWAT reach (subbasin) ID to select from ``output.rch``.
        obs_id : str
            Column name identifying the observed streamflow series.
        flow_col_num : int, default 6
            Zero-based ``output.rch`` column containing simulated flow.
        include_warmup : bool, default True
            Passed to :meth:`get_streamflow_sim` when constructing dates.
        dropna : bool, default True
            Remove dates without both simulated and observed values.

        Returns
        -------
        pandas.DataFrame
            Columns are ``date``, ``stf_sim``, and the requested ``obs_id``.

        Notes
        -----
        This is the preferred public interface. ``get_streamflow_sim_obs`` is
        retained for compatibility with existing iHydroCal scripts.
        """

        return self.get_streamflow_sim_obs(
            obs_file=obs_file,
            obs_col=obs_id,
            subbasin=reach_id,
            flow_col_num=flow_col_num,
